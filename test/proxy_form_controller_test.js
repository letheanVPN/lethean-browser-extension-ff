...
// Stub out the `browser.proxy`, `browser.i18n`, and `browser.extension` APIs
browser = browser || {
  proxy: {
    settings: {
      get: function() {},
...
var proxyform = new Test.Unit.Runner({
  ...
  // from `browser.proxy.settings.get`, and react accordingly.
  // Let's see if that happens with the next four sets of assertions.
  testSetupFormSystem: function() {
    browser.proxy.settings.get = mockFunctionFactory({
      value: {mode: 'system'},
      levelOfControl: 'controllable_by_this_extension'
    });
    ...
    this.wait(100, function() {
      this.assertEqual(
          6,
          browser.proxy.settings.get.getCallList().length);
      this.assert(
